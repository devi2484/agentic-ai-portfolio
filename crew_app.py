...
def evaluate_trust(url: str, company: str = "") -> str:
    ...
    # Advanced Normalization: Matches "THE LEGO GROUP" to "lego.com" flawlessly
    if company:
...
def run_enhanced_search(company: str) -> str:
    ...
    queries = [
        ...
        f"{company} capital allocation infrastructure investments investments {current_year}",
    ...
    try:
        with DDGS() as ddgs:
            for q in queries:
                # Removed restrictive chronological filters to ensure structural profile captures
...
# ==========================================
# 6. PYDANTIC MODELS (HIGH QUALITY PROTOCOLS)
# ==========================================
...
Aim to extract 4-6 deep, descriptive factual markers from the provided text context blocks.

CRITICAL ASSIGNMENT DIRECTIONS: 
1. Map source tracking links into "source_url" and structural trust tags ("HIGH TRUST", "MEDIUM TRUST", "LOW TRUST") into "source_trust" precisely.
2. For "board_relevance" and "strategic_impact", score accurately between 1 and 10. Core operational updates, factory shifts, structural revenue numbers, or major market share changes MUST be scored 8-10 to survive security gates.
...
{{
  "facts": [
    {{
      ...
      "fact": "specific verifiable fact with numbers, percentages, or milestones",
      "source_url": "The exact absolute tracking URL",
      "source_trust": "The exact tracking trust configuration token value",
      "date_signal": "Specific quarter/year. If the text indicates an active ongoing strategy or structural reality, explicitly set this to '{current_year}'",
      "board_relevance": 9,
      "strategic_impact": 9
    }}
  ]
...
{raw_context}"""
    try:
        ...
        for f in data.get("facts", []):
            try: 
                # Cleans mixed data formatting types out of model integers
  ...
  "evaluated_options": [
    {
      ...
      "description": "...",
      "traceability_chain": "...",
      "evidence_support": "High/Medium/Low Metric",
      "risk": "High/Medium/Low Metric",
      "complexity": "High/Medium/Low Metric",
      "strategic_fit": "High/Medium/Low Metric",
